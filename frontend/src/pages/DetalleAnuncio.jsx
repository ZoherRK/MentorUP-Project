...
import { apiGetAnuncio, apiCreateReserva, apiVerificarFecha, apiCreateValoracion, apiValoracionesPendientes } from '../services/api';
...
export default function DetalleAnuncio() {
  ...
  const [fechaCheck, setFechaCheck] = useState(null); // null | 'ok' | 'err'
  ...
  // Verify date availability when fecha_clase changes
  const handleFechaChange = async (value) => {
    setReserva(r => ({ ...r, fecha_clase: value }));
    if (!value || !anuncio?.profesor?.id) return;
    try {
      const res = await apiVerificarFecha(anuncio.profesor.id, value);
      setFechaCheck(res.disponible ? 'ok' : 'warn');
      setCheckMsg(res.mensaje);
    } catch (_) {
      setFechaCheck(null);
      setCheckMsg('');
    }
      ...
      <div style={s.inner}>
        ...
        <div style={s.layout}>
          ...
          <div style={s.main}>
            ...
            {/* Valoración form (alumnos with completed reserva) */}
            {puedeValorar && (
              <div style={s.section}>
            ...
            {/* Availability viewer for students */}
            {anuncio.profesor?.id && (
              <div style={s.section}>
          ...
          <div style={s.sidebar}>
            ...
            {prof && (
              <div style={s.profCard}>
                ...
                {prof.telefono && (
                  <div style={s.profContact}>📞 {prof.telefono}</div>
                )}
              </div>
            )}
            ...
            {isAuth && usuario?.rol === 'alumno' ? (
              <div style={s.bookCard}>
                ...
                {msg.text && (
                  <div style={msg.type === 'ok' ? s.alertOk : s.alertErr}>
                    {msg.text}
                  </div>
                )}
                  ...
                  <form onSubmit={handleReserva} style={s.bookForm}>
                    <div style={s.bookField}>
                      ...
                      <input
                        ...
                        style={{
                          ...s.input,
                          borderColor: fechaCheck === 'ok' ? 'var(--success)' : fechaCheck === 'warn' ? 'var(--warning)' : undefined,
                        }}
                      />
                      {fechaCheck && (
                        <div style={fechaCheck === 'ok' ? s.checkOk : s.checkWarn}>
                          {fechaCheck === 'ok' ? '✅' : '⚠️'} {checkMsg}
                        </div>
                      )}
                    </div>
                    ...
                    <div style={s.bookField}>
                      ...
                      <select
                        ...
                        onChange={e => setReserva(r => ({ ...r, duracion_h: e.target.value }))}
                        style={s.input}
                      >
                    ...
                    <div style={s.priceSummary}>
                      <span style={s.priceSummaryLabel}>Total estimado</span>
                    ...
                    >
                      {submitting ? 'Enviando solicitud…' : `Solicitar clase — ${precioTotal} €`}
                    </button>
                    <p style={s.bookNote}>
                      El profesor recibirá tu solicitud y confirmará la clase.
                    </p>
                  </form>
                )}
              </div>
